---
name: commit
description: Create one or more atomic Conventional Commits from the actual working-tree changes. Use only when the user explicitly asks to commit; analyze the diff, split mixed concerns, explain why, and add no tool attribution.
compatibility: Requires git and an initialized repository.
---

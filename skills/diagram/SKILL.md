---
name: diagram
description: Generate accurate Mermaid diagrams from codebase analysis. Use when explicitly asked to visualize architecture, types, Git history, sequences, or state; return GitHub-native Mermaid or optionally render SVG/PNG with mmdc.
compatibility: Local SVG or PNG rendering requires mmdc; inline Mermaid requires a compatible Markdown renderer.
---
...
Determine what to visualize from the request supplied with this skill invocation. If no scope was supplied and the skill was activated by `pr`, derive the scope from the diff range in §7. Otherwise ask: "What would you like me to diagram?"

The invocation request may identify:
...
4. The `pr` skill embeds these inline in the PR description.

### What `pr`, `visuals`, and `assess` do with diagrams

The installed `visuals` skill orchestrates **what to show** in pull requests and reviews—the Story, Delta, and review Focus—while this skill defines **how to draw it**: types, syntax, size caps, and accessibility. The `pr` skill uses it in authoring mode and `assess` uses it in review mode. All three keep diagrams focused on the change, capped, and preceded by plain-text descriptions.
...
- **Diagrams for Grove.** When activated by the `consolidate` skill, follow that skill's bundled protocol: diagrams are first-class knowledge leaves and should be saved as `.mmd` files in the selected tree's `Knowledge/` directory.

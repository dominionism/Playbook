---
name: visuals
description: Produce the visual-clarity supplement used by the `pr` and `assess` skills. Use when explicitly asked for Story, Delta, or review Focus visuals; generate an end-to-end Mermaid flow, a compact before/after, and risk-focused notes while following the installed `diagram` skill.
---

Make the work legible at a glance. This protocol produces the visual supplements that the `pr` skill embeds in pull requests and the `assess` skill embeds in reviews. The bar is fixed: a developer who has never opened this repo follows the flow without asking anyone, and a developer who knows it cold still learns exactly where the change lives.

Treat the request supplied with this skill invocation as the caller's request. It may be empty; if so, detect the mode per §1.
...
- Aim for 10–20 nodes. Activate the installed `diagram` skill for its hard caps and syntax rules: 30 nodes, 50 edges, with `accTitle` and `accDescr` mandatory.
